...
#ifndef MVAReader_sr_v3_H
#define MVAReader_sr_v3_H

class MVAReader_sr_v3 : public multidraw::TTreeFunction {
public:
  
  MVAReader_sr_v3(const char* model_path, bool verbose, int cut);

  char const* getName() const override { return "MVAReader_sr_v3"; }
  TTreeFunction* clone() const override { return new MVAReader_sr_v3(model_path_.c_str(), verbose, cut_); }
...
MVAReader_sr_v3::MVAReader_sr_v3(const char* model_path, bool verbose, int cut):
...
double 
MVAReader_sr_v3::evaluate(unsigned)
{
  // Run only if 
...
void
MVAReader_sr_v3::bindTree_(multidraw::FunctionLibrary& _library)
{  
