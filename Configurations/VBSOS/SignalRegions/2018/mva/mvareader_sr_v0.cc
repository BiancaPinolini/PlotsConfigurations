...
#ifndef MVAReader_sr_v0_H
#define MVAReader_sr_v0_H

class MVAReader_sr_v0 : public multidraw::TTreeFunction {
public:
  
  MVAReader_sr_v0(const char* model_path, bool verbose, int cut);

  char const* getName() const override { return "MVAReader_sr_v0"; }
  TTreeFunction* clone() const override { return new MVAReader_sr_v0(model_path_.c_str(), verbose, cut_); }
...
protected:  
  ...
  TTreeReaderValue<Double_t>* Zepll{};
};

MVAReader_sr_v0::MVAReader_sr_v0(const char* model_path, bool verbose, int cut):
...
double 
MVAReader_sr_v0::evaluate(unsigned)
{
  // Run only if 
...
void
MVAReader_sr_v0::bindTree_(multidraw::FunctionLibrary& _library)
{  
  ...
  _library.bindBranch(Zepll, "Zeppll_al");
}
