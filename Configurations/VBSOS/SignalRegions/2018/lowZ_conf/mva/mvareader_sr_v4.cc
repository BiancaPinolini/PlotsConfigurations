...
#ifndef MVAReader_sr_v4_H
#define MVAReader_sr_v4_H

class MVAReader_sr_v4 : public multidraw::TTreeFunction {
public:
  
  MVAReader_sr_v4(const char* model_path, bool verbose, int cut);

  char const* getName() const override { return "MVAReader_sr_v4"; }
  TTreeFunction* clone() const override { return new MVAReader_sr_v4(model_path_.c_str(), verbose, cut_); }
...
MVAReader_sr_v4::MVAReader_sr_v4(const char* model_path, bool verbose, int cut):
...
double 
MVAReader_sr_v4::evaluate(unsigned)
{
  // Run only if 
...
void
MVAReader_sr_v4::bindTree_(multidraw::FunctionLibrary& _library)
{  
